...
<head>
    ...
    <style>
       ...
       margin-bottom: auto;
        ...
        .cart-table th, .cart-table td, .cost-table tr {
            padding: 10px;
            text-align: center;
...
.reviewed-star-rating {
    font-size: 35px; /* Adjust the size of the stars */
...
$order = array(
    ...
    "products" => array(
        array(
            ...
            "price" => 1000,
            "quantity" => 1,
            "summary" => "A high-performance laptop suitable for gaming and professional use.",
            "image_link" => "https://encrypted-tbn2.gstatic.com/shopping?q=tbn:ANd9GcQ1JMJed1ZPdVb_R2wlC39fu0Zk8CpL6Q0u4CVzawgyWNcv50o0FX74agozxfHesrpc5iru4WbKD4xNETxq3tBK2OpuAfI0kc0CoH5KNGvbOMxP3fCyFbk6Dqp1WtLfQS7WeMNsEVqSt90&usqp=CAc"
        ),
        array(
            "name" => "Headphone",
            "price" => 1500,
            "quantity" => 2,
            "summary" => "Wireless headphones with noise cancellation for clear sound quality.",
            "image_link" => "https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcRMS0AJi1Ud2iVEoa5dbyNFLXhY9HwQmt5kwfDUZSgng8pAl9OWbm_fyS70I_t_XwIJ_XhSpP1LWn3HDCGq1H4jBp3Suh8_nv2qMobgbG1YdgiNnxgu_VQPCa_sDk5o34j61u9F28MWuw&usqp=CAc"
        ),
        array(
            "name" => "T.v",
            "price" => 50000,
            "quantity" => 1,
            "summary" => "Wireless headphones with noise cancellation for clear sound quality.",
            "image_link" => "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTLOcd2y9vv6E0-GqT-btbOb3xjnp9ErHH4Kw&s"
        ),
    ),
    "subtotal" => 4000,  // Subtotal for all products
    "total" => 4400,     // Total after taxes or other adjustments
    "tax" => 400,        // Tax applied (if applicable)
    "shipping" => 0,     // Shipping fee (if applicable)
    "customer_name" => "John Doe",
    "customer_email" => "john.doe@example.com"
);

function update_order_totals(&$order) {
    $new_subtotal = 0;
    
    // Recalculate the subtotal by adding each product's price * quantity
    foreach ($order['products'] as $product) {
        $new_subtotal += $product['price'] * $product['quantity'];
    }

    // Update subtotal
    $order['subtotal'] = $new_subtotal;
    
    // Recalculate tax (e.g., 10% tax)
    $order['tax'] = $new_subtotal * 0.05;
    
    // Recalculate total
    $order['total'] = $new_subtotal + $order['tax'] + $order['shipping'];
}
update_order_totals($order);

$review_types = [
    'Seller','Product','Delivery','Process'
];
// $order = false;
?>
    ...
    <?php if ($order): ?>
        ...
        <div class="order-summary">
                    ...
                    <?php foreach ($order['products'] as $item): ?>
                        <tr>
                            <td class="product-image">
                                <img src="<?php echo $item['image_link']; ?>" alt="<?php echo $item['name']; ?>">
                            </td>
            ...
            <div class="cost-summary">
                ...
                <table class="cost-table">
                    <tbody>
                        <tr>
                            <td class="cost-item">Subtotal · <?php echo count($order['products']); ?> items</td>
                            <td class="cost-value">₹<?php echo number_format($order['subtotal'], 2); ?></td>
                        </tr>
                      
                        <tr>
                            <td class="cost-item">Tax </td> <!-- Since it's just one tax amount -->
                            <td class="cost-value">₹<?php echo number_format($order['tax'], 2); ?></td>
                        </tr>

                        <tr>
                            <td class="cost-item">Shipping</td>
                            <td class="cost-value">Free</td>
                        </tr>
                        <tr>
                            <td class="cost-item" style="font-weight: 700; color: #333;">Total</td>
                            <td class="cost-value" style="font-weight: 700; color: #333;">₹<?php echo number_format($order['total'], 2); ?></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="review-section">
            <h3>Order Review</h3>
            <?php if ($order && $user_has_reviewed): ?>
                ...
                <form class="review-form" id="reviewForm" method="POST" action="{{ route('review.submit') }}">
                    ...
                    @foreach($review_types as $type)
                        <div  style="padding-bottom:15px;">
                            ...
                            <input type="hidden" name="rating[{{$type}}]" id="rating-{{$type}}" value="0">

                            <!-- Error message container for each type -->
                            <div class="error-message-{{$type}}" style="color: red; display: none; font-size:12px;"></div>
                        </div>
                    @endforeach
...
<?php if ($order): ?>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            @foreach($review_types as $type)
                const stars{{$type}} = document.querySelectorAll('#star-rating-{{$type}} .star');
                const hiddenInput{{$type}} = document.getElementById('rating-{{$type}}');
                const errorMessage{{$type}} = document.querySelector('.error-message-{{$type}}'); // Target error message container for this review type
            
                // Initialize the stars based on the value in the hidden input
                const rating{{$type}} = hiddenInput{{$type}} ? hiddenInput{{$type}}.value : 0;
                ...
                stars{{$type}}.forEach(star => {
                    ...
                    star.addEventListener('mouseout', function() {
                        let selectedValue = hiddenInput{{$type}} ? hiddenInput{{$type}}.value : 0;
                    ...
                    star.addEventListener('click', function() {
                        let value = star.getAttribute('data-value');
                        if (hiddenInput{{$type}}) {
                            hiddenInput{{$type}}.value = value; // Update hidden input with the rating
                        }
            ...
            reviewForm.addEventListener('submit', function(event) {
                ...
                @foreach($review_types as $type)
                    ...
                    if (hiddenInput{{$type}}.value === '0') {
                        formIsValid = false;
                        errorMessage{{$type}}.style.display = 'block'; // Show the error message for this specific type
                        errorMessage{{$type}}.innerHTML = 'Please fill {{$type}}.';
                    } else {
                        errorMessage{{$type}}.style.display = 'none'; // Hide the error message if rating is selected
                    }
                @endforeach
